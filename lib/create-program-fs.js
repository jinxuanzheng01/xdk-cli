...
async function createComponent({name, scope, parentModule, parentPage = {}}) {
    ...
    // 业务文件夹路径
    let component_root = {
        'global': () => path.join(Entry, '/components', name),
        'module': () => path.join(Entry, parentModule, '/components', name),
        'page'  : () => path.join(Entry, parentPage.root, '/pages', parentPage.page, '/components', name)
    }[scope]();
    ...
    // 填充app.json
    scope === 'page' && await writeComponentPageJson(name, parentPage, path.join(Entry, parentPage.root, '/pages', parentPage.page));
