...
async function createComponent({name, scope, parentModule, parentPage = {}}) {
    ...
    // 业务文件夹路径
    let component_root = {
        'global': () => path.join(appRoot, '/component', name),
        'module': () => path.join(appRoot, parentModule, '/component', name),
        'page'  : () => path.join(appRoot, parentPage.root, '/pages', parentPage.page, '/component', name)
    }[scope]();
    ...
    // 填充app.json
    scope === 'page' && await writeComponentPageJson(name, parentPage, path.join(appRoot, parentPage.root, '/pages', parentPage.page));
